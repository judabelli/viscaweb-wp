...
add_action( 'add_meta_boxes', 'sp_create_custom_meta_box', 10, 2 );

function sp_create_custom_meta_box( $post_type, $post ) {
  add_meta_box( 'custom-textbox', __( 'Custom Meta Box', 'sb-custom-meta-box' ), 'sp_custom_meta_box_content', 'page', 'side', 'high' );
}

function sp_custom_meta_box_content() {
$json = file_get_contents('http://viscaweb.com/developers/test-front-end/pages/step2-sportsbooks.json');
$array = json_decode($json);
$meta_element_class = get_post_meta($post->ID, 'sb-custom-meta-box', true);
?>
<div class="form">
    <label> <?php _e('Sportsbook', 'twentytwenty'); ?> </label>
  <select class="sb-choicelist" name="sb-choicelist">
  <?php foreach($array as $value => $name)  {?>
    <option value="<?php echo $value ?>" <?php selected( $meta_element_class, $value); ?>><?php echo $name ?></option>
  <?php } ?>
  </select>
</div>
<?php
}
function sp_save($post_id) {
global $post;
    if(isset($_POST["sb-choicelist"])){
        $meta_element_class = $_POST['sb-choicelist'];
        update_post_meta($post->ID, 'sb-custom-meta-box', $meta_element_class);
    }

}
add_action('save_post', 'sp_save');
